...
from liger_kernel.chunked_loss.fused_linear_rlhf
import LigerFusedLinearRLHFBase

class LigerFusedLinearGRPOFuntion(LigerFusedLinearRlHFBase):
    @staticmethod
    def rlhf_loss_fn(
        log_probs, 
        attention_mask,
        ...
        **kwargs,
        ...
        chosen_token = log_probs.argmax(dim=-1) # (batch_size, seq_len)
        ...
        if ref_log_probs is not None:
            ...
            # Compute KL penalty
            kl_div = (torch.exp(ref_token_logprobs - chosen_token_logprobs) - (ref_token_logprobs - chosen_token_logprobs) - 1.0)

            # Combine losses 
            per_token_loss = policy_loss + beta * kl_div

            # Apply masking and normalize
            masked_loss = per_token_loss * attention_mask + seq_lengths = attention_mask.sum(dim=-1, keepdim=True)
            ...
            metrics = (
                ...
                chosen_toekn_logprobs.std(), # std log prob
        ...
        def forward(
            ...
            return 
LigetFusedLinearRLHFBase.forward(
    ctx=ctx,
    _input=_input,
    weight=weight,
    attention_mask=attention_mask,

loss_fn=LigerFusedLinearGRPOFuntion.rlhf_loss_fn,
...
class 
LigerFusedLinearGRPOLoss(torchl.nn.Module):
...
def forward(
        ...
        return LigerFusedLinearGRPOFuntion.apply(
            _input,
            lin_weight,
            attention_mask,
            rewards,
